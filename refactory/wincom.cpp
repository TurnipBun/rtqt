...
int WinCom::open()
{
    ...
    if (comm == INVALID_HANDLE_VALUE) return COM_ERR_OPEN;
...
int WinCom::send(const string& data)
{
    ...
    if (!ret || written != len) return COM_ERR_SEND;
    ++sendCount;
    return COM_SUC;
}
...
int WinCom::recv(string& data)
{
    ...
    if (!ClearCommError(comm, &err, &status)) return COM_ERR;
    if (status.cbInQue == 0) return COM_ERR_RECV;
    memset(readBuf,'a',READBUF_LEN);
    readBuf[READBUF_LEN-1] = '\0';
    ret = ReadFile(comm, readBuf, READBUF_LEN, &recvedSize, NULL);
    if(!ret || recvedSize == 0)  return COM_ERR_RECV;
